...
if ($query === '') {
    // Si la query es vacía, devolvemos las 5 ciudades más importantes por defecto
    $defaults = array($airports[0], $airports[1], $airports[2], $airports[3], $airports[4]);
...
foreach ($airports as $airport) {
    ...
    $idStr = (string)$airport['id'];
    
    // Buscar coincidencia en el ID, en el código IATA o en el nombre visible
    if (
        strpos($idStr, $normalizedQuery) !== false ||
        strpos($normCode, $normalizedQuery) !== false ||
        strpos($normText, $normalizedQuery) !== false
    ) {
        $matches[] = $airport;
    }
}

// Limitar los resultados al tamaño solicitado por la app (por defecto 5)
$size = isset($_GET['size']) ? intval($_GET['size']) : 5;
if ($size <= 0) {
    $size = 5;
}

$result = array_slice($matches, 0, $size);
